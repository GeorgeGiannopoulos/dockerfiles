...
fix_embedded_db() {
    # HACK [issue #12]: In the current version of the code, there is an issue with auto-setup, where
    #      the EmbeddedConnectionProvider is not appended in the openfire.xml on the first boot.
    #      In case, the instance reboots, it fails to connect due to the above. Replacing the
    #      pattern "DefaultConnectionProvider" with "EmbeddedConnectionProvider" fixes the issue at
    #      the moment.
    #      In case the openfire is set up manually the issue does not exist.
    echo "Fixing Embedded DB..."
    config="${OPENFIRE_DATA_DIR}/conf/openfire.xml"
    if [[ -f "${config}" ]]; then
        if grep -q "DefaultConnectionProvider" "${config}"; then
            echo "Replacing DefaultConnectionProvider with EmbeddedConnectionProvider"
            sed -i 's/DefaultConnectionProvider/EmbeddedConnectionProvider/g' "${config}"
        fi
    fi
}
...
# openFire daemon
DAEMON_OPTS="-server                                            \
             -DopenfireHome=${DAEMON_DIR}                       \
             -Dopenfire.lib.dir=${DAEMON_LIB}                   \
             -Dlog4j.configurationFile=${DAEMON_LIB}/log4j2.xml \
             -classpath ${DAEMON_LIB}/startup.jar               \
             -jar ${DAEMON_LIB}/startup.jar ${EXTRA_ARGS}"
